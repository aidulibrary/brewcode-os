...
| 级别 | 定义 | 修复时限 | 示例 |
|------|------|----------|------|
| **P0 阻断** | 核心功能不可用、数据错误、安全/隐私风险 | 立即 | 首页 404、支付按钮失效、白皮书链接 404、i18n 渲染错误导致内容回退到旧版 |
...
- **对称修复**：如必须调整内容，改中文必须同步英文（i18n 字典 + HTML 字面量）。**本次验收不做任何文案延伸修改。**
...
- [ ] 标题、Hero 副标题、首页主要内容与 git HEAD 逐字一致（仅做一致性验证，不做文案修改）
- [ ] 白皮书链接 200 且内容正确
