...
function generateShareCard(brewData, options) {
  ...
  return renderWithScale(scale)
    .then(function (canvas) {
      ...
      injectShareCardCSS();
      return renderWithScale(scale * 0.9).then(function (canvas2) {
        if (canvas2.height <= 6000) {
          cleanup();
          return canvas2;
        }
        // 仍然溢出：0.8 倍缩放
        ...
        container = c3;
        injectShareCardCSS();
        return renderWithScale(scale * 0.8).then(function (canvas3) {
          cleanup();
          return canvas3;
        });
      });
