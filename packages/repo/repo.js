...
function seedManifestToBrewData(entry) {
  ...
  if (c.producer) {
    coffee.producer = c.producer;
  }
  if (c.roaster) {
    coffee.roaster = c.roaster;
  }
  ...
  if (e.kettle) {
    equipment.kettle = e.kettle;
  }
  if (e.scale) {
    equipment.scale = e.scale;
  }
  if (e.filter) {
    equipment.filter = e.filter;
  }
...
function buildStepsHTML(brewData) {
  ...
  for (var i = 0; i < steps.length; i++) {
    ...
    if (stepText) {
      ...
      html +=
        '<div class="share-card-step">' +
        '<span class="share-card-step-num">' +
        escHTML(num) +
        '</span>' +
        '<span class="share-card-step-text">' +
        escHTML(text) +
        '</span>' +
        '</div>';
    }
...
function generateShareCard(brewData, options) {
  ...
  container.style.cssText = 'position:fixed;left:-9999px;top:0;width:640px;z-index:-1;';
