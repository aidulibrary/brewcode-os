  ...
  if (submitBtn) {
    submitBtn.addEventListener('click', function () {
      ...
      var payload = {
        ...
        dose: typeof p.dose === 'object' ? (p.dose.value||0) : (p.dose||0), waterAmount: typeof p.waterAmount === 'object' ? (p.waterAmount.value||0) : (p.waterAmount||0), ratio: p.ratio||null,
        grindSize: typeof p.grindSize === 'object' ? ((p.grindSize.value||'').toString()) : (p.grindSize||''), waterTemperature: typeof p.waterTemperature === 'object' ? (p.waterTemperature.value||null) : (p.waterTemperature||null),
